...
function domscripts_linkify(sInputText, sTitleLogic, bTargetBlank) {
    ...
    sReturn = sReturn.replace(replacePattern00, '<img class="domscripts_linkify" src="$2">'); // eg .domscripts_linkify { height:20pt; width: 20pt; }

    // replace the 𝓣search term𝓣 here - more math alphabet shortcuts like this?
    replacePattern00 =  /𝓣(.*?)𝓣/gim; 
    // sReturn = sReturn.replace(replacePattern00, '<a href="https://theoryoftheory.github.io/searchtree.htm?search=$1">𝓣$1</a>');
    sReturn = sReturn.replace(replacePattern00, function(sOuterMatch,sInnerMatch,iPositionMatching) {
        // console.log(sInnerMatch.replace('"', '%22').replace("'", "%27"));
        return `<a href="https://theoryoftheory.github.io/searchtree.htm?search=${sInnerMatch.replace('"', '%22').replace("'", "%27")}">𝓣${sInnerMatch}</a>`;
    });
    ...
    sReturn = sReturn.replace(replacePattern00, function(sMatch, sMatches, s3, s4) {
    ...
    sReturn = sReturn.replace(replacePattern1, function(sMatch, sMatch2_ElectricBoogaloo, sSecretThirdParameter, sSecretFourth) { // what are these secret parameters in a regex.replace's callback function?
        ...
        return `<a href="${sMatch.replace('"', '%22').replace("'", "%27")}" ${sTargetBlank}>${sHrefTitle}</a>`;
    } );

    // URLs starting with "www." (without // before it, or it'd re-link the ones done above).
    replacePattern2 = /(^|[^\/])(www\.[\S]+(\b|$))/gim;
    // sReturn = sReturn.replace(replacePattern2, '$1<a href="http://$2" target="_blank">$2</a>');
    sReturn = sReturn.replace(replacePattern2, '$1<a href="http://$2" ' + sTargetBlank + '>$2</a>');
